...
export function ReportPanel({ hideTotals = false }: { hideTotals?: boolean } = {}) {
  ...
  const sumTransfer = paidRows.reduce(
    (s, r) => s + Number(r.advance_transfer) + Number(r.final_transfer),
    0,
  );
  ...
  const zoneCash = (list: BillingLog[]) =>
    list.reduce((s, r) => s + Number(r.advance_cash) + Number(r.final_cash), 0);
  const zoneTransfer = (list: BillingLog[]) =>
    list.reduce((s, r) => s + Number(r.advance_transfer) + Number(r.final_transfer), 0);
  ...
  const sumProductTransfer = paidProductSales.reduce(
    (s2, x) => s2 + Number(x.paid_transfer ?? 0),
    0,
  );
  ...
  const chartData = useMemo(() => {
    ...
    for (const r of chartRows) {
      ...
      if (map.has(key)) {
        const v =
          Number(r.advance_cash) +
          Number(r.final_cash) +
          Number(r.advance_transfer) +
          Number(r.final_transfer);
    ...
    <div>
      <div className="row g-2 align-items-center mb-3">
        ...
        {mode === "day" ? (
          <>
            <div className="col-auto">
              <label className="form-label small fw-bold m-0">วันที่:</label>
            </div>
            <div className="col-auto">
              <input
                type="date"
          ...
          <>
            ...
            <div className="col-auto">
              <label className="form-label small fw-bold m-0">ถึง:</label>
            </div>
            <div className="col-auto">
              <input
                type="date"
